function [d, points, angles] = distanceOutline2(x1,y1,heading1,x2,y2,heading2,ship1, ship2)
...
%P1 jeszcze raz żeby zamknąć krzywą
    ...
    for j=1:l2
        ...
        if b2^2 > b1^2 + a^2
            if d>b1
                ...
                dxA=outlineA(i-1,1)-outlineA(i+1,1);
                dyA=outlineA(i-1,2)-outlineA(i+1,2);
                
                dxB=outlineB(j-1,1)-outlineB(j+1,1);
                dyB=outlineB(j-1,2)-outlineB(j+1,2);
                angleA=
                angleB=
                d=b1;
            end
