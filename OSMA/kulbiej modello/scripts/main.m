...
for i = 1:dt:t+1
    ...
    for q=1
        ...
        for q=1
            ...
            Xhull = Xhull_nd * 0.5*water_Density*length*length*U*U;
            ...
            Xprop = water_Density * propellerRotation * propellerRotation * propellerDiameter* propellerDiameter* propellerDiameter* propellerDiameter * propellerKT;
            %rudder, ster
            ...
            Xwind = 0;
            XSW = 0;
            Xgeneral = m*w0(3)*v0(2) + m*xG*(w0(3)*w0(3)+w0(2)*w0(2))-m*v0(3)*w0(2)-m*zG*w0(1)*w0(3)+water_Density*9.81*A_WP*deltaZ*sin(r0(2)/57.3);
            Xgeneral = 0;
        ...
        for q=1
            ...
            Yhull = Yhull_nd * 0.5*water_Density*length*length*U*U;
            Yprop = 0;
            Yrud = 1.5*F_Rudder * cos(RUDDER/57.3);
            Ywind = 0;
            YSW = 0;
            Ygeneral = m*(w0(1)*v0(3) - w0(3)*v0(1)-zG*w0(1)*w0(3)-xG*w0(1)*w0(2))- water_Density*9.81*A_WP*deltaZ*sin(r0(1)/57.3)*cos(r0(2)/57.3);
            Ygeneral = 0;
        ...
        for q=1
            ...
            Nhull = Nhull_nd * 0.5*water_Density*length*length*length*U*U;
            Nprop = 0;
            ...
            ng_1 = m*xG*v0(3)*w0(1);
            ng_2 =-xG*v0(1)*w0(3)*m;
            ng_3 =xG*zG*w0(2)*w0(3)*m;
            ng_4 = (Ixx-Iyy)*w0(1)*w0(2);
            ng_5 =-m*9.81*(-GML0*cos(r0(2)/57.3)+GMT0)*sin(r0(2)/57.3)*sin(r0(1)/57.3);
            Ngeneral =ng_1 + ng_2 + ng_3 + ng_4 + ng_5;
            Ngeneral = 0;
...
figure;
hold on;
plot(wyniki(:,13),wyniki(:,12));
legend('rot_Z');
%spradek prêdkoœci na cyrkulacji
figure;
hold on;
plot(wyniki(:,13),wyniki(:,3))
legend('U/U_0');
hold off;
figure;
hold on;
plot(wyniki(:,13),wyniki(:,4))
plot(wyniki(:,13),wyniki(:,5))
...
% figure;
% hold on;
% plot(wyniki(:,13),wynikiF(:,2))
% plot(wyniki(:,13),wynikiF(:,3))
% plot(wyniki(:,13),wynikiF(:,4))
% plot(wyniki(:,13),wynikiF(:,5))
% legend('Xhull','Xres','Xprop','Xrudder');
% hold off;
%si³y Y
figure;
hold on;
plot(wyniki(:,13),wynikiF(:,10))
plot(wyniki(:,13),wynikiF(:,11))
plot(wyniki(:,13),wynikiF(:,12))
plot(wyniki(:,13),wynikiF(:,10)+wynikiF(:,11)+wynikiF(:,12))
plot(wyniki(:,13),wynikiBX(:,2))
legend('Yhull','Yrudder', 'Ygeneral','Ytotal','Ytotal');
hold off;
%momenty N
figure;
hold on;
plot(wyniki(:,13),wynikiF(:,21))
plot(wyniki(:,13),wynikiF(:,22))
plot(wyniki(:,13),wynikiF(:,23))
plot(wyniki(:,13),wynikiBX(:,6))
