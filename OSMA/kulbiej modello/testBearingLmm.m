...
cpaRequired = 0.5*(a.breadth+b.breadth);
elapsedTime=0;
exit=true;
simulationEndTime=50;
maxElapsedTime=500;
distanceLimit = 50*a.length;
%% pętla
koniec=false;
collision = false;
simulationEnd = false;
...
while true
    ...
    if collision
        ...
        lmmX = - aX + bX;
        lmmY = - aY + bY;
        disp("Znaleziono graniczny punkt LMM, lmmX = " + lmmX + ", lmmY = " + lmmY + ".");
        break;
    end
    if distanceSimulated > distanceLimit
        simulationEnd = true;
        disp("Osiągnięto limit symulacji.");
        break;
    end
end
...
a = aRef; b = bRef;
b.x = lmmX; %distanceSimulated*cos(deg2rad(bearing));
b.y = lmmY; %distanceSimulated*sin(deg2rad(bearing));
...
[distPBU1,results]=distanceOutline(aRef.x,aRef.y,aRef.heading,bRef.x,bRef.y,bRef.heading,aRef,bRef);
distPBU2=distance(aRef,bRef);
if nnn > cpaRequired
    disp(['Zasymulowany manewr ostatniej chwili w wariancie ' ...
        ...
        ' m, wymagano ' num2str(cpaRequired) ' m, odległość minięcia między punktami to ' num2str(wyniki(zzz,5)) ' m. '...
        ...
        'Pozycja punktu bez ucieczki dla statku własnego to P = (' num2str(aRef.x) ' m,' num2str(aRef.y) ' m)'...
    ...
    disp(['Niespełnione, zbliżono na ' num2str(nnn) ' m, oczekiwano ' num2str(cpaRequired) ' m, odleglość między punktami to ' num2str(wyniki(zzz,5)) ' m. ']);
