function c = rysujWykresyZigZag(bezWymiarowo,monitor, dane, vessel, HDGCOG, path, outlines, offset, velocities, accelerations, ...
        X, Y, N, weather, zigZag, aisData, offsetZigZag, offsetZigZagX, offsetZigZagY)
    time = length(dane(:,1));
    timeDimentionLess = zeros(time,1);
    for i=1:time
        timeDimentionLess(i,1)=i*vessel.vx/vessel.length;
    end
    for i=1:length(aisData(:,5))
        if aisData(i,5)>180
            aisData(i,5)=aisData(i,5)-360;
        end
    end
    if HDGCOG==true
    ...
    if path == true
        ...
        plot((aisData(offsetZigZag:time+offsetZigZag-1,2)+offsetZigZagX)/scale...
            ,(aisData(offsetZigZag:time+offsetZigZag-1,3)-offsetZigZagY)/scale);
        legend('simulated', 'EXP');
        if outlines == 1
            for i=1:time
                if mod(i,offset)==0
                    outline=drawOutline(dane(i,3),dane(i,2),dane(i,4),vessel.length,vessel.breadth,scale);
    ...
    if zigZag == true
        ...
        if bezWymiarowo
            ...
            plot(timeDimentionLess(:,1),aisData(offsetZigZag:time+offsetZigZag-1,5));
            xlabel("t' [-]");
        ...
        legend('rudder','heading', 'heading EXP');
