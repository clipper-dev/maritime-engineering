...
global  f RX_p RY_k ro_a W v_a gamma_r V RY_r RZ_k L RZ_r RX_k RX_r m moment opory
L=statek.L;
B=statek.B;
...
%skladowe od kadluba zalezne od rodzaja stattu sa inne wzory
...
%skladowe  wiatru zalezne od rodzjau statku sa inne wzroy
u_wr=-pr_wzdl-v_a*cos(gamma_r-kurs_r); 
v_wr= pr_pop- v_a*sin(gamma_r-kurs_r);
beta_ra= atan(v_wr/u_wr);
...
k_rudx= 1.5*atan(wych_steru-6.5)-1.7; %% w zaleznosci od kata wychylenia steru: do i od 10stopni, czy plus czy minus %% tu dla wiecej ni¿ 10stopni
k_rudy= 0.6;
k_rudn= 1.4;
h_rud= wys_steru; % wysokosc steru
c_rud= 1.65; % ciêciwa steru na oko
u_rud=sqrt(0.77*(u_p+0.7*u_al)^2 +0.23^u_p*u_p);
v_rud= pr_pop-0.5*pk_r*L;
beta_rud=-atan(v_rud/u_rud);
pow_steru=0.7; % wielkosc na oko
F_rud=0.5* ro_w * pow_steru *u_rud^2*(6.13 *h_rud/ c_rud)/(h_rud/ c_rud+ 2.25);
a_H=0.176;
RX_r=-k_rudx* F_rud * sin(wych_steru+0.8*beta_rud);
RY_r= k_rudy*(1+a_H)*F_rud*cos(wych_steru+0.8*beta_rud);
RZ_r= -k_rudn*0.5*L*RY_r;

%opory=[opory; RX_k RY_k RZ_k RX_a RY_a RZ_a RX_p RY_p RZ_p RX_r RY_r RZ_r]; 
f=zeros(6,1);
f(1)=x; f(2)=y; f(3)=kurs_r; f(4)=pk_r; f(5)=pr_wzdl; f(6)=pr_pop;
...
[czas,f]=ode45('rigid', [ts-dt ts], war_pocz, options);% warunek poczatkowy zmienic
